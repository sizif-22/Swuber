...
public class BookedShuttleBuses extends JPanel {
  ...
  public BookedShuttleBuses(Frame frame, User user) throws SQLException {
    ...
    // Add Cards here
    int counter = 0;
    String sql = "select s.* from ShuttleRide s, scheduledRides c where s.id = c.shuttleId and c.userId ="
        + user.getUserId() + ";";
    ...
    while (resultSet.next()) {
      ...
      contentPanel.add(new BookedShuttleCard(counter++, startLocation, endLocation, price));
    }
