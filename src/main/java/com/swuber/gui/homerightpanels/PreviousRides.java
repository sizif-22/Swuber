...
import com.swuber.functionality.Ride;
...
public class PreviousRides extends JPanel {
    public PreviousRides(Frame frame, User user) {
        ...
        // Add Cards here
        int counter = 0;
        for (Ride ride : user.getRideHistory()) {
            contentPanel.add(new RidesCard(ride.getStartLocation(), ride.getEndLocation(), ride.getDriver().getName(),
                    ride.getRating(), counter++));
        }
