...
public class BookedShuttleBuses extends JPanel {
  ...
  public BookedShuttleBuses(Frame frame, User user) throws SQLException {
    this.user = user;
    this.frame = frame;
    ...
    // Add Cards here
    int counter = 0;
    // String sql = "select s.* from ShuttleRide s, scheduledRides c where s.id =
    // c.shuttleId and c.userId ="
    // + user.getUserId() + ";";
    ...
    for (ShuttleRide ride : ShuttleRide.getShuttleRidesForUser(user)) {
      contentPanel
          .add(new BookedShuttleCard(counter++, ride.getStartLocation(), ride.getEndLocation(), ride.getPrice()));
    }
    // }
