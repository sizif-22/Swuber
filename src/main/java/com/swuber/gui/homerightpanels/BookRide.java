...
public class BookRide extends JPanel {
	...
	public BookRide(Frame frame, User user) {
		...
		// System.out.println("Drivers in System: " +
		// RidePlanner.getAllDrivers().size());
		...
		bookRideBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				Ride ride = null;
				try {
					ride = Ride.addRide(user, pickupLocation, destination);
				} catch (Exception e1) {
				...
				try {
					...
					matchedDriver = Driver.matchDriverToRide(ride);
					if (matchedDriver != null & ride != null) {
						...
						// loader :( )
						frame.gotoPaymentPanel(user, ride, null);
					} else {
